...
import * as UserLoggerTypes from "../userlogger.types";
...
export const logActions = {
  LOGIN: (payload: UserLoggerTypes.LoginLoggerPayload) => {
    ...
    logger.info(
      `${
        dev
          ? `User [ID: ${id}] logged in from ${payload.source}`
          : "User logged in"
      }`,
    );
  },
  VERIFY: (payload: UserLoggerTypes.VerifyLoggerPayload) => {
    ...
    logger.info(
      `${
        dev && user
          ? `User [ID: ${user.id}] verified as still logged in`
  ...
  LOADING: (payload: UserLoggerTypes.LoadingLoggerPayload) => {
    if (dev) {
      logger.info(
        `Loading started for ${payload.process} at ${new Date().toISOString()}`,
      );
    }
  },
